...
var refreshUiWrapped=false,refineTimer=null,pageObserver=null;
...
function setCommitments(rows){
  commitments=new Map();
  (Array.isArray(rows)?rows:[]).forEach(function(c){commitments.set(String(c.lead_id),c);});
  queueRefine();
}
function syncCommitments(){
  try{var a=window.TM_COMMITMENTS;if(a&&typeof a.state==='function'){setCommitments(a.state());return true;}}catch(e){}
  return false;
}
...
function abrirCancelar(c){
  ...
  d.querySelector('[data-confirm]').onclick=function(){
    var r=d.querySelector('#tm-direct-cancel-reason').value;if(!r){toast('Selecione o motivo.');return;}
    var b=d.querySelector('[data-confirm]');b.disabled=true;
    rpc('request_followup_commitment_cancel',{p_commitment_id:c.commitment_id,p_reason:r})
      .then(function(){d.remove();toast('Agendamento cancelado. O Admin será notificado.');return window.CRM_FOLLOWUP&&typeof window.CRM_FOLLOWUP.carregar==='function'?window.CRM_FOLLOWUP.carregar():null;})
      .then(function(){var a=window.TM_COMMITMENTS;return a&&typeof a.refresh==='function'?a.refresh():null;})
      .catch(function(e){b.disabled=false;toast(String(e&&e.message||e));});
  };
}
...
function boot(){
  css();instalarCadencia();syncAtualizar();instalarRefreshUi();syncCommitments();queueRefine();
  ...
  var tries=0,t=setInterval(function(){tries++;instalarRefreshUi();syncAtualizar();if(refreshUiWrapped||tries>80)clearInterval(t);},100);
  document.addEventListener('tm:commitment-state',function(ev){setCommitments(ev&&ev.detail&&ev.detail.state||[]);});
  document.addEventListener('tm:followup-rendered',function(){syncCommitments();queueRefine();});
  ...
  window.addEventListener('message',function(ev){if(ev.source===window&&ev.data&&ev.data.type==='CRM_UPDATED')setTimeout(queueRefine,250);});
}
